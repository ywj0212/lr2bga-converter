...
# ────────────────────────────── 편의 위젯 (옵션) ──────────────────────────────
def add_text_i18n(key: str, *, tag: Optional[str] = None, parent: Optional[str] = None, fmt: dict | None = None, **kwargs) -> str:
    txt = t(key, **(fmt or {}))
    _tag = tag or f"i18n_txt_{id(key)}_{len(_BINDINGS)}"
    dpg.add_text(txt, tag=_tag, parent=parent, **kwargs)
    bind_value(_tag, key, **(fmt or {}))
    return _tag

def add_button_i18n(key: str, *, tag: Optional[str] = None, parent: Optional[str] = None, callback=None, fmt: dict | None = None, **kwargs) -> str:
    lbl = t(key, **(fmt or {}))
    _tag = tag or f"i18n_btn_{id(key)}_{len(_BINDINGS)}"
    dpg.add_button(label=lbl, tag=_tag, parent=parent, callback=callback, **kwargs)
    bind_label(_tag, key, **(fmt or {}))
    return _tag

def add_checkbox_i18n(key: str, *, tag: Optional[str] = None, parent: Optional[str] = None, default_value=False, callback=None, fmt: dict | None = None, **kwargs) -> str:
    lbl = t(key, **(fmt or {}))
    _tag = tag or f"i18n_chk_{id(key)}_{len(_BINDINGS)}"
    dpg.add_checkbox(label=lbl, tag=_tag, parent=parent, default_value=default_value, callback=callback, **kwargs)
    bind_label(_tag, key, **(fmt or {}))
    return _tag
